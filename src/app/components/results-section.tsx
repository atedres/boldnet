    ...
    return (
        <section className="py-16 md:py-24 bg-red-700/20 text-white backdrop-blur-sm">
            <div className="container mx-auto px-4">
                <SectionTitle className="mb-16">{content?.title?.[language] || content?.title || "LES RÉSULTATS?"}</SectionTitle>
                <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto mt-12 relative">
                    ...
                    <div className="relative z-10">
                        <div className="text-center mb-4">
                            <span className="font-bold text-xl rounded-full px-6 py-2 bg-black text-white inline-block shadow-lg">{content?.withoutTitle?.[language] || content?.withoutTitle || "Avant Boldnet"}</span>
                        </div>
                        <Card className="bg-black/20 p-6 rounded-lg border border-red-500 text-center h-full">
                            <CardContent className="p-0 mt-4 space-y-4 text-red-200">
                                {(content?.withoutItems || []).map((item: any, i: number) => <p key={i} className="text-lg">{item?.[language] || item}</p>)}
                    ...
                    <div className="relative z-10">
                        <div className="text-center mb-4">
                            <span className="font-bold text-xl rounded-full px-6 py-2 bg-white text-red-600 inline-block shadow-lg">{content?.withTitle?.[language] || content?.withTitle || "Après Boldnet"}</span>
                        </div>
                        <Card className="bg-white/90 p-6 rounded-lg text-gray-800 text-center h-full">
                            <CardContent className="p-0 mt-4 space-y-2">
                                {(content?.withItems || []).map((item: any, i: number) => <p key={i} className="text-lg">✓ {item?.[language] || item}</p>)}
                                <div className="relative aspect-video mt-4 rounded-lg overflow-hidden">
                ...
                <div className="max-w-4xl mx-auto text-center mt-20">
                    <p className="font-bold text-xl md:text-2xl"><span className="bg-white text-red-600 px-3 py-1 rounded-md">Bonus:</span> {content?.bonus?.[language] || content?.bonus || "Votre impact grandit aussi. Plus de personnes profitent de votre expertise et de vos services."}</p>
                    <Button onClick={handleOpenForm} size="lg" className="mt-8 rounded-full bg-white text-red-600 hover:bg-gray-200 font-bold text-lg px-10 py-6">
                        {content?.ctaButtonText?.[language] || content?.ctaButtonText || "COMMENÇONS!"} <ArrowRight className="ml-2 h-5 w-5" />
                    </Button>
                </div>
            </div>
