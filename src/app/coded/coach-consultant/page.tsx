...
.bnlp .cookiebar.is-open{transform:translateY(0)}
.bnlp .cookiebar .inner{max-width:1180px;margin-inline:auto;display:flex;align-items:center;
  gap:clamp(16px,3vw,34px);flex-wrap:wrap;justify-content:space-between}
.bnlp .cookiebar p{margin:0;font-size:14.5px;line-height:1.6;color:var(--muted);max-width:64ch}
.bnlp .cookiebar a{color:var(--red-lite);text-decoration:underline;text-underline-offset:3px}
.bnlp .cookiebar .acts{display:flex;gap:10px;flex:none}
.bnlp .cookiebar button{font-family:var(--f-mono);font-size:11px;letter-spacing:.16em;
  text-transform:uppercase;padding:13px 22px;border:0;cursor:pointer;transition:background .25s,color .25s}
.bnlp .cookiebar .yes{background:var(--red);color:#fff}
...
@media (max-width:1000px){
  ...
  .bnlp .flow div{padding:26px 0 0 0}
  .bnlp .flow div+div{padding-left:0;border-left:0;border-top:1px solid var(--line);margin-top:26px}
  .bnlp .flow div+div::before{left:0}
  .bnlp .quotes{grid-template-columns:1fr}
  .bnlp .shot-ph{aspect-ratio:16/9}
  .bnlp .price{grid-template-columns:1fr}
  .bnlp .price-side{border-left:0;border-top:1px solid var(--line)}
  .bnlp .price-list{grid-template-columns:1fr}
  .bnlp .deliv{grid-template-columns:repeat(2,1fr)}
...
export default function CoachConsultantPage() {
  useEffect(() => {
    ...
    /* --- tracking : chargé uniquement si un ID est renseigné ET si le visiteur a consenti --- */
    ...
    /* --- bannière de consentement -----------------------------------------
       Aucun script publicitaire n'est chargé avant un « Accepter » explicite.
       Le choix est mémorisé 6 mois dans le stockage local du navigateur ;
       il se retire depuis /confidentialite.html.
       ---------------------------------------------------------------------- */
    (function(){
      const KEY = 'bnlp_consent';
      const A = CONFIG.analytics || {};
      const hasTracker = !!(A.ga4 || A.metaPixel || A.tiktokPixel);
      const bar = document.getElementById('cookiebar');
      if (!bar) return;

      const MAX_AGE = 180 * 24 * 60 * 60 * 1000; /* 6 mois, comme annoncé dans la politique */

      /* Format stocké : "granted|1712345678901" — la valeur puis la date du choix. */
      function read(){
        try {
          ...
          const value = parts[0];
          const when  = parseInt(parts[1], 10);
          if (!when || (Date.now() - when) > MAX_AGE) {
            localStorage.removeItem(KEY);   /* choix expiré : on redemande */
            return null;
          }
          return value;
        } catch (e) { return null; }
      ...
      function write(v){
        try { localStorage.setItem(KEY, v + '|' + Date.now()); } catch (e) { /* navigation privée */ }
      }
      function hide(){
        bar.classList.remove('is-open');
        window.setTimeout(function(){ bar.hidden = true; }, 240);
      }

      /* Rien à tracker : la bannière n'a pas lieu d'être. */
      if (!hasTracker) { bar.hidden = true; return; }
      ...
      bar.hidden = false;
      /* laisse un frame au navigateur pour animer l'entrée */
      ...
      if (yes) yes.addEventListener('click', function(){
        write('granted'); hide(); loadTrackers();
      });
    ...
    /* --- provenance de la visite ------------------------------------------
       Relève les paramètres UTM et le fbclid de l'URL au premier chargement,
       les garde le temps de l'onglet (le visiteur peut passer par les pages
       légales et revenir : l'URL a alors perdu ses paramètres), puis les joint
       à la demande. Objectif : savoir dans le Sheet quelle annonce a produit
       quel lead, sans dépendre uniquement du rapport de Meta.
       ---------------------------------------------------------------------- */
    const TRAFFIC = (function(){
      ...
      let stored = null;
      try { stored = JSON.parse(sessionStorage.getItem(KEY) || 'null'); } catch (e) { /* navigation privée */ }
      ...
      KEYS.forEach(function(k){
        ...
        if (v) { fresh[k] = String(v).slice(0, 200); found = true; }
      });

      /* Une visite avec des paramètres écrase ce qui était mémorisé :
         c'est la campagne qui vient de l'amener qui compte. */
      if (found) {
      ...
      return { referrer: (document.referrer || '').slice(0, 200) };
    })();
  ...
  return (
    <div className="bnlp">
      ...
      <div className="cookiebar" id="cookiebar" role="dialog" aria-live="polite"
           aria-label="Consentement aux cookies de mesure" hidden>
        <div className="inner">
          <p>
            On utilise des cookies de mesure pour savoir quelle campagne vous a amené ici. Rien n'est
            chargé tant que vous n'avez pas accepté, et refuser ne change rien à la page.{' '}
            <a href="/confidentialite.html">Politique de confidentialité</a>
          </p>
          <div className="acts">
            <button type="button" className="no" id="cookieNo">Refuser</button>
            <button type="button" className="yes" id="cookieYes">Accepter</button>
          </div>
        </div>
      </div>
